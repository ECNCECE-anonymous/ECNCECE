...
def eval_random(genome, config):
    return random.uniform(0,1)

#@profile
def eval_genome(genome, config, learn=True, total_timesteps=100_000, skip_evaluated=True, n_seasons=3, seed=10301):
    #global env
...
def run(config_file='neat_config_exp_v2', parallel=False, wandb_run=None, restore=False, checkpoint_file=None):
    ...
    if parallel:
        ...
        pe = ParallelEvaluator(2, eval_genome, timeout=800)
        #pe = ParallelEvaluator(40, eval_random, timeout=500)
        ...
        for generation in range(gen_start, 400):
            ...
            if wandb_run:
                ...
                gen_best_task_perf = gen_best.fitness + 1 #TODO change for energy experiments
                wandb_run.log({"gen": p.generation-1, "gen_best_fitness": gen_best.fitness, "gen_mean_fitness": gen_mean, 
